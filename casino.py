...
class Game():
	def __init__(self, house: House, decks: int = 6, guests:int = 4, warning_card:int = 30):
		self.house = house
		...
		self.dealer = Dealer(cash = 10_000_000_000.00)
	...
	def run_table(self):
		while self.rounds_played < self.house.rounds_per_table and [player.cash for player in self.players if player.cash > 0]:
			...
			if self.rounds_played%100 == 0:
				self.house_profit[(self.rounds_played//100)-1] = 10_000_000_000 - self.dealer.cash

class Player():
	count = 0
	def __init__(self, cash = 1_000_000_000.0, spot_count = 1, normal_bet=15, hand_fee=0.25):
	...
	#def pay_fee(self):
		#if self.normal_bet == 2:
			#if self.is_dealer == False:
				#self.cash -= self.hand_fee
			#elif self.is_dealer == True:
				#self.cash += 1.50
